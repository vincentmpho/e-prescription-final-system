...
export default function Edit() {
    ...
    useEffect(() => {
        fetch(`https://localhost:44345/api/FirstVisits/${id}`)
            .then(res => res.json())
            .then(res => {
                setChronicHistory(res.chronicHistory)
                setDoctorNumber(res.doctorNumber)
                setPatientNumber(res.knownAllegies.split(";")[2])
                setCurrentChronicMedication(res.currentChronicMedication)
                setKnownAllegies(res.knownAllegies.split(";")[0])
                setVisitDate(res.knownAllegies.split(";")[1])
    ...
    return (
        <div className="container">
            ...
            <h4>Edit First visit details</h4>
            <hr />
